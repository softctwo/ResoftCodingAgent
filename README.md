...
npm run resoft dashboard -p 8080 --host 0.0.0.0  # 自定义端口+地址

# ─── 用量统计 ───
npm run resoft stats summary         # 总览
npm run resoft stats daily           # 7 日趋势
npm run resoft stats daily -d 30     # 30 日趋势
npm run resoft stats pricing         # 模型成本对比
npm run resoft stats export -o usage.json
